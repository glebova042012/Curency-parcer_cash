...
WORKSHEET_NAME = "CASH_RUB_TJS_daily"
# -----------------

def get_rub_rates() -> Tuple[Dict[str, float], str]:
    """
    Парсит курсы наличной продажи RUB/TJS на ТЕКУЩУЮ дату.
    Возвращает:
    ...
    try:
        ...
        col_sell_cash = headers_list.index("Наличные продажа")
    ...
    for row in rows:
        ...
        sell_raw = cells[col_sell_cash].get_text(strip=True)
        try:
...
def save_to_gsheet(bank_rates: Dict[str, float], data_datetime: str) -> None:
    ...
    try:
        ...
        ws.append_row(["datetime_request", "date_rate", "bank", "cash sell_rate"])
